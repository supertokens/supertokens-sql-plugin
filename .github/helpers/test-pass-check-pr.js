...
const linkToJob =
  "https://github.com/supertokens/supertokens-sql-plugin/actions/workflows/tests.yml";
...
function doJob() {
  ...
  const ownerAndRepoString = process.env.REPO;
  const sourceOwnerAndRepoString = `${process.env.SOURCE_OWNER}/${
    ownerAndRepoString.split("/")[1]
  }`;

  axios
    .get(
      `https://api.github.com/repos/${sourceOwnerAndRepoString}/actions/runs?branch=${process.env.BRANCH}`,
      {
        headers: {
    ...
    .then(async (result) => {
      ...
      if (failed) {
        console.log(
          "Test job failed. Exiting... Please rerun this job manually when you run the test job again..."
        );
        process.exit(1);
        return;
      }
      ...
      console.log(
        'You need to trigger the "' +
          testJobName +
          '" github action and make this job succeed'
      );
      console.log("You can find the github action here: " + linkToJob);
      if (Date.now() - startTime > 1.5 * 60 * 1000) {
        console.log(
          "Test job not started after waiting for 1.5 mins... Exiting, please rerun this job manually."
        );
        process.exit(1);
      } else {
