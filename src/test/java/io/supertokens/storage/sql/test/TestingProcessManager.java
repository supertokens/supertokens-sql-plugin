...
class TestingProcessManager {
    ...
    public static abstract class TestingProcess extends Thread {
        ...
        io.supertokens.storage.sql.ProcessState.EventAndException checkOrWaitForEventInPlugin(
                io.supertokens.storage.sql.ProcessState.PROCESS_STATE state) throws InterruptedException {
            return checkOrWaitForEventInPlugin(state, 15000);
        }

        io.supertokens.storage.sql.ProcessState.EventAndException checkOrWaitForEventInPlugin(
                io.supertokens.storage.sql.ProcessState.PROCESS_STATE state, long timeToWaitMS)
                throws InterruptedException {
            Start start = (Start) StorageLayer.getStorage(main);
            io.supertokens.storage.sql.ProcessState.EventAndException e = io.supertokens.storage.sql.ProcessState
            ...
            if (e == null) {
                ...
                while (e == null && (System.currentTimeMillis() - startTime) < timeToWaitMS) {
                    Thread.sleep(100);
                    e = io.supertokens.storage.sql.ProcessState.getInstance(start).getLastEventByName(state);
                }
            }
            return e;
        }
